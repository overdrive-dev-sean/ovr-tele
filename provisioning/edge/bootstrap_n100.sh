...
Identity:
  ...
  --system-id <id>          Optional system_id (defaults to node-id)
...
load_existing_defaults() {
  local existing_env="${OVERDRIVE_DIR}/site.env"
...
apply_defaults() {
  ...
  if [ -z "${REMOTE_WRITE_PASSWORD_FILE}" ]; then
    REMOTE_WRITE_PASSWORD_FILE="${OVERDRIVE_DIR}/secrets/remote_write_password"
  fi
  ...
  if [ -z "${VM_WRITE_PASSWORD_FILE}" ]; then
    VM_WRITE_PASSWORD_FILE="${OVERDRIVE_DIR}/secrets/vm_write_password"
  fi
}
...
prompt_for_values() {
  ...
  prompt_optional SYSTEM_ID "System ID" \
    "Hardware/system label (defaults to node-id)." "${SYSTEM_ID:-${NODE_ID}}"
  ...
  if [ -n "${REMOTE_WRITE_URL}" ]; then
    ...
    local rw_default_file="${REMOTE_WRITE_PASSWORD_FILE:-${OVERDRIVE_DIR}/secrets/remote_write_password}"
  ...
  if [ "${HAS_GX}" = "true" ]; then
    if [ -z "${GX_HOST}" ]; then
      GX_HOST="192.168.100.2"
    fi
    prompt_optional GX_HOST "GX host" \
      "GX IP address (default: 192.168.100.2)." "${GX_HOST}"
  fi
...
write_site_env() {
  mkdir -p "${OVERDRIVE_DIR}/secrets"
  chmod 0700 "${OVERDRIVE_DIR}/secrets"
  ...
  if [ -n "${MAPBOX_TOKEN}" ]; then
    if [ -z "${MAPBOX_TOKEN_FILE}" ]; then
      MAPBOX_TOKEN_FILE="${OVERDRIVE_DIR}/secrets/mapbox_token"
    fi
  ...
  cat > "${OVERDRIVE_DIR}/site.env" <<EOF
...
VM_WRITE_PASSWORD_FILE=${VM_WRITE_PASSWORD_FILE:-${OVERDRIVE_DIR}/secrets/vm_write_password}
VM_WRITE_PASSWORD=${VM_WRITE_PASSWORD}

VM_REMOTE_WRITE_URL=${REMOTE_WRITE_URL}
VM_REMOTE_WRITE_USERNAME=${REMOTE_WRITE_USER}
VM_REMOTE_WRITE_PASSWORD_FILE=${REMOTE_WRITE_PASSWORD_FILE:-${OVERDRIVE_DIR}/secrets/remote_write_password}

MAPBOX_TOKEN=${MAPBOX_TOKEN}
MAPBOX_TOKEN_FILE=${MAPBOX_TOKEN_FILE:-${OVERDRIVE_DIR}/secrets/mapbox_token}

GX_HOST=${GX_HOST}
GX_SSH_PORT=
GX_USER=root
GX_PASSWORD_FILE=${OVERDRIVE_DIR}/secrets/gx_password
EOF

  chmod 0600 "${OVERDRIVE_DIR}/site.env"
}
...
generate_targets() {
  ...
  if [ -n "${TARGETS_FILE}" ]; then
    cp "${TARGETS_FILE}" "${OVERDRIVE_DIR}/targets.yml"
    return 0
  fi
...
EOF
    ...
    } > "${OVERDRIVE_DIR}/targets.yml"
    return 0
  fi

  if [ -f "${EDGE_DIR}/examples/targets.yml.example" ]; then
    local gx_target_host="${GX_HOST:-192.168.100.2}"
    sed \
      -e "s/__SYSTEM_ID__/${SYSTEM_ID}/g" \
      -e "s/Pro6005-2/${SYSTEM_ID}/g" \
      -e "s/__GX_HOST__/${gx_target_host}/g" \
      "${EDGE_DIR}/examples/targets.yml.example" \
      > "${OVERDRIVE_DIR}/targets.yml"
  else
    cat > "${OVERDRIVE_DIR}/targets.yml" <<EOF
- targets:
    - "192.168.100.2:9480"
  labels:
    job: gx_fast
...
generate_vmagent_config() {
  ...
  local dst="${OVERDRIVE_DIR}/vmagent.scrape.yml"
  if [ ! -f "${src}" ]; then
...
ensure_acuvim_targets() {
  local file="${OVERDRIVE_DIR}/targets_acuvim.txt"
  if [ ! -f "${file}" ]; then
    cat > "${file}" <<'EOF'
...
check_required_files() {
  ...
  if [ "${HAS_GX}" = "true" ]; then
    local gx_file="${OVERDRIVE_DIR}/secrets/gx_password"
    if [ ! -f "${gx_file}" ]; then
...
deploy_stack() {
  ...
  local release_env="${OVERDRIVE_DIR}/edge.env"
  if [ -f "${release_env}" ]; then
    echo "Deploying EDGE using release compose (edge.env found)..."
    set -a
    # shellcheck disable=SC1090
    . "${release_env}"
    set +a
    ...
    echo "Deploying EDGE using dev compose (no ${release_env} found)..."
