...
Copy `/tmp/ovr-debs` to the USB (e.g. `/cdrom/ovr/debs`). Legacy media may still use `/cdrom/overdrive/debs`.
...
Legacy media also reads `overdrive/` for these paths.
...
`site.env` or `firstboot.env` (e.g., `WIFI_PASS_FILE=/etc/ovr/secrets/wifi_password`).

## Per-node files

- `/etc/ovr/site.env` (identity + remote write + GX credentials)
...
it preserves any existing values from `/etc/ovr/site.env`.
...
Update `/etc/ovr/site.env` or `/etc/ovr/targets.yml`, then restart:
