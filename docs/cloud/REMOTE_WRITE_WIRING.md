...
docker compose -f compose.dev.yml run --rm caddy \
  caddy hash-password --plaintext 'your_password'
```

Put the hash into `cloud/.env` (symlink to `/etc/ovr/cloud.env`) as `VM_WRITE_PASS_HASH`.

## N100 config (edge.env)

Set these in `/etc/ovr/edge.env`:
...
- `deployment_id` and `node_id` from `/etc/ovr/edge.env` (external labels)
