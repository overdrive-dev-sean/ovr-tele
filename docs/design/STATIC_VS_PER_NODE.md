...
| --- | --- | --- | --- |
| Docker compose topology | `edge/compose.dev.yml` | Static | Keep identical across nodes; inject values via `/etc/ovr/site.env`. |
...
| Node identity | (new) `/etc/ovr/site.env` | Per-node | `DEPLOYMENT_ID`, `NODE_ID`, `SYSTEM_ID`, remote write creds. |
...
| GX IP + SSH creds | `edge/services/events/app.py` env | Per-node | `GX_HOST`, `GX_USER`, `GX_PASSWORD` in `/etc/ovr/site.env`. |
| Telegraf global tags | `edge/telegraf/telegraf.conf` | Per-node values | Uses env vars from `/etc/ovr/site.env`. |
